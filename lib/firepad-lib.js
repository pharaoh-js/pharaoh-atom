/* **********************************************************************************
 * Firepad 1.3.0 firepad.io ||  MIT License, Copyright 2014 Firebase ****************
 * Code from ot.js (Copyright 2012-2013 Tim Baumann) ||| Major additions by gh:fank *
 * Major modifications by gh:zacanger ***********************************************
 * Search in text for ATOM INTEGRATION, oldlib, and HACK, and TODO ******************
 ***********************************************************************************/

(function (name, definition, context) {
  // attempt to use commonjs, then amd, then global
...
firepad.TextOp = (function() {
  ...
  // operations: lists of ops. three types:
  // retain ops: advance cursor position by N chars; positive ints
  // insert ops: insert given string at cursor position; strings
  // delete ops: delete next N chars; negative ints
...
firepad.TextOperation = (function () {
  ...
  function TextOperation () {
    ...
    // when operation is applied to input str, it's kind of like an imaginary
    // cursor runs over the str and skips over some parts, deletes some parts,
    // and inserts at some positions. these skips, deletes, and inserts, are
    // all stored as an array in the 'ops' property.
    // an operation's baseLength is the length of every string to which the
    // operation can be applied.
    // targetLength is the length of every string that that _results_ from
    // applying the operation on a valid str.
  ...
  // after operation is constructed, user (of the lib) can specify the
  // actions (skip, insert, delete) with these builder methods. they return
  // the operation (for easy chaining).
  ...
  TextOperation.prototype.retain = function (n, attributes) {
    ...
    if (prevOp && prevOp.isRetain() && prevOp.attributesEqual(attributes)) {
      // last op is a retain op with same attributes => can be merged into one op
      prevOp.chars += n;
    } else {
      // new op
  ...
  // insert str at current position
  ...
  // delete str at current position
  ...
  // tests whether operation has no effect
  ...
  TextOperation.prototype.toString = function () {
    // map: build new array by applying function to every element in old array
  ...
  TextOperation.prototype.toJSON = function () {
    ...
    // return array with /something/ in it, since firebase treats empty array as null
    if (ops.length === 0) {
      ops.push(0);
    }
    return ops;
  ...
  // converts plain js obj into an operation and validates it
  ...
  TextOperation.prototype.apply = function (str, oldAttributes, newAttributes) {
    ...
    for (var i = 0, l = ops.length; i < l; i++) {
      ...
      if (op.isRetain()) {
        ...
        // copy skipped part of the retained str
        newStringParts[j++] = str.slice(oldIndex, oldIndex + op.chars);

        // copy (and potentially update) attributes for each char in retained str
...
firepad.FirebaseAdapter = (function (global) {
  ...
  FirebaseAdapter.prototype.setUserId = function(userId) {
    if (this.userRef_) {
      // clean up existing data, don't break new data if new user takes our old name
      this.userRef_.child('cursor').remove();
...
// oldlib: user2Callback might not be needed anymore
//   var user2Callback = { };
...
//zz HERE BEGINS THE ACTUAL ATOM INTEGRATION. ENDS AT 2861.
...
firepad.Firepad = (function(global) {
  ...
  function Firepad(ref, place, options) {
    ...
    if (!CodeMirror && !ace && !atom) {
      throw new Error('Couldn\'t find CodeMirror, ACE, or Atom. Did you forget something?');
    }
   ...
   // this.registerBuiltinEntities_();
    ...
    // oldlib, maybe replaces line below?
  ...
  Firepad.prototype.setText = function(textPieces) {
    ...
    if (this.ace_) {
      ...
      this.codeMirror_.refresh();
    } else if (this.atom_){  //oldlib--unsure
...
/* bigass thing from oldlib, would replace starting at `var doc` in the following section.
